...
struct SListObject {
  2: required i64 size,
  3: required i32 compressedHead, // Pointer to the head element
  // TODO(bwatling): remove the default value and clean up SList::SList() once
  // we can rely on 'compressedTail' always being valid.
  4: i32 compressedTail = -1, // Pointer to the tail element
}

struct DListObject {
  1: required i32 compressedHead,
  2: required i32 compressedTail,
  3: required i32 size,
}
